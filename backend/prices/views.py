...
def should_scrape():
    now = datetime.now(pytz.timezone('Europe/Istanbul'))  # Türkiye saati
    weekday = now.weekday()  # 0 = Pazartesi, 5 = Cumartesi, 6 = Pazar
    
    if weekday >= 5:  # Cumartesi veya Pazar → scrape etme
        return False
    
    hour = now.hour
    # Yaklaşık bakım arası (Bigpara/spot piyasalarda yaygın): gece 00:30-01:30 arası atla
    # (Test edip net saati bul, bazı yerlerde 01:00-02:00)
    if (hour == 0) or (hour == 1):  
        return False  # Gece saatlerinde scrape'i atla (güvenli tarafta kal)
        ...
        try:
            ...
            if not latest_record or latest_record.created_at < timezone.now() - timedelta(hours=1):
                if should_scrape():
                    ...
                    print("Weekend or maintenance hour → skipping scrape")
