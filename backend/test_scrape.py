...
# Clear old duplicate data before saving new ones
...
for row in rows:
    ...
    # Exact mapping based on the debug output
    gold_type = None
    if name == "ALTIN (TL/GR)":
        gold_type = "gram"
...
# Print the saved records from the database
for gold in GoldPrice.objects.all():
    print(f"{gold.gold_type} | Buy: {gold.price_buy} | Sell: {gold.price_sell}")